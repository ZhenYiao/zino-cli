...
pub fn new_parse() ->anyhow::Result<()> {
    ...
    let project_name = dialoguer::Input::<String>::new()
        .with_prompt(ansi_term::Color::Cyan.paint("Enter the project name:").to_string())
        .interact()?;
    ...
    let selections = &[
        ansi_term::Color::RGB(255,128,0).paint("Actix App").to_string(),
        ansi_term::Color::RGB(255,255,0).paint("Axum App").to_string(),
        ansi_term::Color::RGB(128,255,0).paint("Ntex App").to_string(),
        ansi_term::Color::RGB(0,255,255).paint("dioxus-ssr").to_string(),
        ansi_term::Color::RGB(127,0,255).paint("dioxus-desktop").to_string(),
    ];
    let selection = dialoguer::Select::with_theme(&theme)
        .with_prompt(ansi_term::Color::Cyan.paint("Select the project template:").to_string())
        .items(&selections[..])
        .default(0)
        .interact()
        .unwrap();
